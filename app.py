...
# --- 2. ฟังก์ชันช่วยงาน (Logic ใหม่ที่ปรับปรุงแล้ว) ---
...
def extract_numbers(text):
    """สูตรแกะตัวเลขฉบับบ่อทอง (ปรับปรุงใหม่)"""
    # ลบตัวอักษรที่ชอบอ่านผิด
    text = text.replace('O', '0').replace('o', '0') 
    
    # ดึงตัวเลขทั้งหมดออกมา
    ...
    # ขั้นตอนที่ 1: คัดกรองเบื้องต้น
    for num_str in numbers:
        val = int(num_str)
        # กรองเลขขยะออก (Voltage, Hz, ปีผลิต, มอก.)
        if val in [220, 50, 15, 45, 2023, 2024, 2025, 2552, 2336]:
            continue
        # เก็บเลขที่มี 3 หลักขึ้นไป
        if len(num_str) >= 3:
            candidates.append(num_str)

    # ขั้นตอนที่ 2: ตามหาเลขห้อง (เน้นหา 4 หลักที่ขึ้นต้นด้วย 10)
    for c in candidates:
    ...
    # ถ้าไม่เจอ 4 หลัก ให้ลองหาแบบ 3 หลัก (ค่าน้อยกว่า 500)
    ...
    # ขั้นตอนที่ 3: ตามหาเลขมิเตอร์ (เอาตัวเลขที่เหลือที่ค่ามากที่สุด)
    if candidates:
...
if sh is None:
    ...
    if img_file:
        ...
        with st.spinner('🤖 AI กำลังอ่านค่า (สูตรใหม่)...'):
            raw_text = get_text_from_image(bytes_data)
            ai_room, ai_reading = extract_numbers(raw_text)
        
        # แสดงผลการอ่านแบบ Debug (เผื่ออยากดูว่ามันเห็นเลขอะไรบ้าง)
        with st.expander("ดูข้อความดิบที่ AI อ่านได้"):
            st.text(raw_text)
            
        st.success("อ่านค่าเรียบร้อย!")
