...
# --- 2. สูตรแกะตัวเลข V.3 (แก้เรื่องเลขหมุน และ Serial Number) ---
...
def extract_numbers(text, m_type):
    """
    สูตรแกะตัวเลข V.3
    - m_type: ประเภทมิเตอร์ ('น้ำประปา' หรือ 'ไฟฟ้า')
    """
    ...
    # เทคนิคพิเศษ: ถ้าเจอเลขที่มีเว้นวรรคคั่นกลาง เช่น "1 4 3 0" ให้รวมเป็น "1430"
    # (AI ชอบอ่านเลขมิเตอร์หมุนๆ เป็นเลขแยกกัน)
    # ลบช่องว่างออกเพื่อให้ regex จับกลุ่มตัวเลขได้ดีขึ้น
    text_merged = text_clean.replace(" ", "") 
    
    # ดึงชุดตัวเลขทั้งหมดออกมาจากทั้งแบบมีเว้นวรรคและไม่มี
    numbers_raw = re.findall(r'\d+', text_clean) # หาจากแบบเดิม
    numbers_merged = re.findall(r'\d+', text_merged) # หาจากแบบรวม
    
    all_candidates = set(numbers_raw + numbers_merged) # รวมกันและตัดตัวซ้ำ
    ...
    ignore_list = [
        220, 50, 15, 45, 100, 400, # หน่วยไฟฟ้า
        2023, 2024, 2025, 2552, 2336, # ปีและ มอก.
        2124, 65057, 6505, # *** เลข Serial Number ที่ AI ชอบอ่านผิด ***
        1, 2, 33 # เลขรุ่นเล็กๆ
    ]

    # --- ขั้นตอนที่ 1: หาเลขห้อง (Priority 1) ---
    # ห้องต้องเป็น 4 หลัก และขึ้นต้นด้วย 10 (เช่น 1018, 1020)
    for num_str in all_candidates:
        if len(num_str) == 4 and num_str.startswith("10"):
            suggested_room = num_str
            break # เจอแล้วจบเลย
    
    # ถ้าไม่เจอ ลองหาแบบ 3 หลัก (กรณีหอพักทั่วไป)
    if not suggested_room:
        for num_str in all_candidates:
            if len(num_str) == 3 and int(num_str) < 500: # สมมติห้องไม่เกิน 500
                suggested_room = num_str
                break

    # --- ขั้นตอนที่ 2: หาเลขมิเตอร์ ---
    for num_str in all_candidates:
        # ตัดเลขห้องที่เจอไปแล้วออก
        if num_str == suggested_room:
            continue
            
        # ข้ามเลขที่สั้นเกินไป (ต่ำกว่า 3 หลัก ไม่น่าใช่มิเตอร์)
        if len(num_str) < 3: 
            continue
        ...
        # *** กฎเหล็ก: ตัดเลขที่ดูเหมือน Serial Number (ยาวเกิน 6 หลัก) ***
        if len(num_str) > 6:
            continue
            
        meter_candidates.append(val)

    # --- ขั้นตอนที่ 3: เลือกเลขมิเตอร์ที่ดีที่สุด ---
    if meter_candidates:
        if m_type == 'ไฟฟ้า':
            # ไฟฟ้า: มักจะมี 5 หลัก (เช่น 14613) ให้ความสำคัญกับเลข 5 หลักก่อน
            priority_candidates = [x for x in meter_candidates if 10000 <= x <= 99999]
            if priority_candidates:
                suggested_meter = max(priority_candidates) # เอาค่ามากสุดในกลุ่ม 5 หลัก
            else:
                suggested_meter = max(meter_candidates) # ถ้าไม่มีเอามากสุดปกติ
                
        else: # น้ำประปา
            # น้ำ: มักจะมี 4 หลัก (เช่น 1430) ให้ความสำคัญกับเลข 4 หลักก่อน
            priority_candidates = [x for x in meter_candidates if 1000 <= x <= 9999]
            if priority_candidates:
                suggested_meter = max(priority_candidates)
            else:
                # กรณีน้ำหมุน (อ่านได้ 143) มันจะเป็น 3 หลัก
                # ให้เลือกตัวที่ดูเป็นไปได้มากสุด
...
if sh is None:
    ...
    if img_file:
        ...
        with st.spinner('🤖 AI กำลังแกะรอยตัวเลข (สูตร V.3)...'):
            # ส่งประเภทมิเตอร์เข้าไปด้วย เพื่อช่วยตัดสินใจ
        ...
        # ปุ่มกดดูเบื้องหลัง (เผื่ออยากรู้ว่า AI เห็นอะไรบ้าง)
